...
const StepperControl = ({handleClick, currentStep, steps}) => {
  return (
    <div className='flex justify-between mx-10 mt-4 mb-4'>
        ...
        {/* next button */}
        <button onClick={()=>handleClick("next")} className='bg-gradient-to-r from-[#3224AFCC] via-[#3927B1] to-[#C46BD6] text-white capitalize py-2 px-4 rounded-md font-semibold cursor-pointer'>{currentStep == steps.lenght -1 ? "Confirm" : "Next"}</button>
    </div>
  )
} 
